[[Java/并发编程/多线程下 上下文信息丢失问题|多线程下 上下文信息丢失问题]] 在 Java 并发编程中，认证信息、用户上下文、traceId 等常通过 `ThreadLocal` 存储在主线程。但当任务切换到异步线程、线程池、Disruptor 或 Redis Stream 等环境时，`ThreadLocal` 数据不会自动传递，导致认证失败、日志丢链等问题。
